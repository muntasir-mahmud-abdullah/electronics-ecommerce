...
import { ChevronRight, ChevronDown, Minus, ChevronLeft } from "lucide-react";
...
function ProductsContent() {
  ...
  const getPageTitle = () => {
    switch(category) {
      case "laptops": return "Computing & Rigs";
      case "smartphones": return "Mobile Devices";
      case "audio": return "High-Res Audio";
      case "monitors": return "Professional Displays";
      default: return "All Hardware";
    }
  };
    ...
    <div className="bg-[#F9FAFB] min-h-screen pb-20">
      <div className="max-w-[1320px] mx-auto px-6 py-8">
        ...
        <div className="flex items-center gap-2 text-[11px] text-[#6B7280] mb-6">
          ...
          <Link href="/products" className="hover:text-[#111827] transition-colors">Hardware</Link>
          {category && (
            <>
              <ChevronRight className="w-3 h-3" />
              <span className="font-semibold text-[#111827]">{getPageTitle()}</span>
            </>
          )}
        </div>
          ...
          <aside className="w-full lg:w-[240px] flex-shrink-0 space-y-4">
            {loadingFilters ? (
              <div className="space-y-4">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="bg-white border border-[#E5E7EB] rounded-xl p-5 h-40 animate-pulse" />
                ))}
              </div>
            ) : filters.length === 0 ? (
              <div className="text-sm text-gray-500 bg-white p-4 border border-[#E5E7EB] rounded-xl">No dynamic filters available for this category.</div>
            ) : (
              filters.map((filter) => (
                <div key={filter.groupId} className="bg-white border border-[#E5E7EB] rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    {filter.options.map((opt: any) => {
                      const isActive = searchParams.get(filter.title) === opt.label;
                      return (
                        <label 
                          key={opt.valueId} 
                        ...
                        >
                          <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${isActive ? 'bg-[#00D4E8] border-[#00D4E8]' : 'bg-white border-[#D1D5DB] group-hover:border-[#00D4E8]'}`}>
                            {isActive && <div className="w-1.5 h-1.5 bg-white rounded-sm" />}
                          </div>
                          <span className={`text-[13px] transition-colors flex-1 ${isActive ? 'text-[#111827] font-semibold' : 'text-[#4B5563] group-hover:text-[#111827]'}`}>
                            {opt.label}
                          </span>
                        </label>
                      );
          ...
          <main className="flex-1">
            {loadingProducts ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="bg-white border border-[#E5E7EB] rounded-xl p-4 h-[340px] animate-pulse flex flex-col" />
                ))}
              </div>
            ) : products.length === 0 ? (
              <div className="text-center py-20 bg-white border border-[#E5E7EB] rounded-xl">
                ...
                {(searchParams.toString() !== "" && searchParams.toString() !== `category=${category}`) && (
                  <button 
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#F9FAFB] flex items-center justify-center text-[#111827] font-bold">Loading hardware catalog...</div>}>
      <ProductsContent />
    </Suspense>
  );
}
