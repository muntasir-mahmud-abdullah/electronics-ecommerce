      ...
      <div className="max-w-[800px] mx-auto px-6">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[#6B7280] text-[15px]">
            Thank you for your purchase. Your order number is <span className="font-bold text-[#111827]">{order.orderNumber}</span>.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[14px] font-bold text-[#111827]">Shipping Address</h2>
            </div>
          ...
          <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-[14px] font-bold text-[#111827]">Order Info</h2>
            </div>
            <div className="text-[13px] text-[#4B5563] space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-[#111827]">{order.paymentMethod}</span>
              </div>
              <div className="flex justify-between">
                <span>Payment Status</span>
                <span className="font-semibold text-[#F59E0B]">{order.paymentStatus}</span>
              </div>
              <div className="flex justify-between">
                <span>Fulfillment Status</span>
                <span className="font-semibold text-[#3B82F6]">{order.fulfillmentStatus}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 mb-8">
            ...
            {order.items.map((item) => (
              <div key={item.id} className="flex gap-4">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <h4 className="text-[13px] font-bold text-[#111827]">{item.productName}</h4>
                  {item.variantLabel && <p className="text-[11px] text-[#6B7280] mt-0.5">{item.variantLabel}</p>}
                  <div className="flex justify-between items-center mt-1">
                    <span className="text-[12px] font-medium text-[#6B7280]">Qty: {item.quantity}</span>
                    <span className="text-[13px] font-bold text-[#111827]">${Number(item.subtotal).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-3 pt-5 border-t border-[#E5E7EB] mb-5">
            <div className="flex justify-between text-[13px]">
              ...
              <span className="font-semibold text-[#111827]">${Number(order.subtotal).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-[13px]">
          ...
          <div className="flex justify-between items-center pt-5 border-t border-[#E5E7EB]">
            ...
            <span className="text-[24px] font-black text-[#00D4E8]">${Number(order.total).toLocaleString()}</span>
          </div>
        </div>
