...
import { Product, Brand, ProductVariant, ProductMedia, AttributeValue, ProductAttribute, AttributeGroup } from "@prisma/client";
...
export function ProductCard({ product }: { product: PopulatedProduct }) {
  ...
  return (
    <Link href={`/products/${product.slug}`} className="flex-shrink-0 w-[240px] bg-white border border-[#E5E7EB] rounded-lg overflow-hidden hover:border-[#00D4E8]/50 transition-colors group flex flex-col">
      <div className="relative h-[200px] bg-[#F9FAFB] flex items-center justify-center">
        ...
        <div className="w-full h-full bg-[#F4F5F7] flex items-center justify-center mix-blend-multiply transition-transform duration-500 group-hover:scale-105">
          {primaryMedia ? (
            <img src={primaryMedia.url} alt={product.name} className="w-full h-full object-contain p-4" />
          ) : (
      ...
      <div className="flex-1 gap-1 flex p-3 flex-col">
        ...
        <div className="flex flex-nowrap py-1 overflow-x-auto scrollbar-none gap-1.5">
          {specs.map((s) => (
            <span
              key={s}
              className="text-[10px] whitespace-nowrap px-1 rounded-sm bg-gray-100 border border-[#E5E7EB] text-[#4B5563] font-medium"
            >
              {s}
            </span>
          ))}
        ...
        <button className="mt-1 w-full bg-[#0A0C14] hover:bg-[#1E2235] text-white font-semibold text-[13px] py-1.5 rounded-lg transition-colors"
          onClick={(e) => { e.preventDefault(); /* Cart action will be wired later */ }}
        >
          View Details
        </button>
      </div>
